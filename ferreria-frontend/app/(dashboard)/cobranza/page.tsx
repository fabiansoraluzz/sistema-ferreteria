...
const METODOS_PAGO = ["Efectivo", "Yape", "Plin", "Transferencia", "Cheque"];
...
export default function CobranzaPage() {
    ...
    async function registrarPago() {
        ...
        if (!monto || montoNum <= 0) {
            alerta.mostrar("Ingresa un monto valido mayor a cero", "error");
            return;
        }
        if (montoNum > cuentaActiva.saldoPendiente) {
            alerta.mostrar("El monto no puede ser mayor al saldo pendiente", "error");
            return;
        }
        setGuardando(true);
        try {
            ...
            alerta.mostrar("Hubo un error al registrar el pago. Intenta de nuevo.", "error");
        } finally {
            setGuardando(false);
        }
    }
    ...
    return (
        <>
                ...
                {cuentas.length > 0 ? (
                    <div className="bg-red-500 rounded-2xl p-5 text-white shadow-sm">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <p className="text-base font-semibold opacity-90">Total pendiente de cobro</p>
                        </div>
                                ...
                                <div key={c.pedidoId} className="px-4 py-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1 min-w-0">
                                            <p className="text-base font-bold text-slate-800 truncate">{c.nombreCliente}</p>
                                        ...
                                        <div className="text-right ml-3 shrink-0">
                                            <p className="text-2xl font-bold text-red-600">S/ {c.saldoPendiente.toFixed(2)}</p>
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-t-3xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl font-bold text-slate-800">{cuentaActiva.nombreCliente}</p>
                            </div>
                            <button
                                onClick={cerrarPago}
                        ...
                        <div>
                            <p className="text-base font-bold text-slate-700 mb-2">Cuanto esta pagando ahora?</p>
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xl font-bold text-slate-500">S/</span>
                                <input
                                    type="number"
                                    value={monto}
                        ...
                        <div>
                            <p className="text-base font-bold text-slate-700 mb-2">Como paga?</p>
                        ...
                        <div>
                            <p className="text-base font-bold text-slate-700 mb-2">Observaciones (opcional)</p>
                            <input
                                type="text"
                                value={observaciones}
            ...
            <ModalConfirmacion
                visible={confirmarPago}
                titulo="Confirmar este pago?"
                descripcion={`Se registrara un pago de S/ ${parseFloat(monto || "0").toFixed(2)} mediante ${metodoPago} para ${cuentaActiva?.nombreCliente ?? ""}`}
                textoConfirmar="Si, registrar pago"
                colorConfirmar="verde"
                cargando={guardando}
                onCancelar={() => setConfirmarPago(false)}
                onConfirmar={() => { setConfirmarPago(false); registrarPedido(); }}
            />
        </>
    );
