...
export default function DetalleClientePage() {
    ...
    async function cargarCliente() {
        try {
            const { data } = await api.get<ClienteDetalle>(
                `/Clientes/ObtenerDetalleCliente/${params.id}`
            );
            setCliente(data);
        } catch {
    ...
    const pedidosConDeuda = cliente.detallePedidos.filter(
        (p) => p.saldoPendiente > 0
    );
